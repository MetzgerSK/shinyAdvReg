ui <- fluidPage(
    ...
    titlePanel(HTML("MLE: The Intuition &ndash; Multinomial Logit")), 
        ...
        tabPanel("Main", value="main_true",
            sidebarLayout(
                sidebarPanel(
                    ...
                    conditionalPanel("input.dataGenButton != 0",
                        ...
                        div(id="bHalf",
                            ...
                            div(class="catParams",
                                h4("Category A"),
                                sliderInput("A_aHat", label = "Intercept \\(\\left(\\alpha_A \\right) \\)", 
        									min = -5, max = 5, step = 0.25, value = 0),
    							sliderInput("A_b1Hat", label = HTML("<em>x</em>'s Coeff \\(\\left(\\beta_A \\right) \\)"), 
    										min = -5, max = 5, step = 0.25, value = .5)
                            ),
    						div(class="catParams",
    							h4("Category B"),
    							sliderInput("B_aHat", label = "Intercept \\(\\left(\\alpha_B \\right) \\)", 
    										min = -5, max = 5, step = 0.25, value = -0.75),
    							sliderInput("B_b1Hat", label = HTML("<em>x</em>'s Coeff \\(\\left(\\beta_B \\right) \\)"), 
    										 min = -5, max = 5, step = 0.25, value = 1.5)
							),
                            
							p(em("Reference Outcome: Category C"), style="font-size:90%;"),
                            br(),
                ...
                mainPanel(
                    ...
                    conditionalPanel("input.dataGenButton != 0",
                        h4("Proposed line equations:"),
                        uiOutput("eq_lm"),
                        p("\\( \\text{* Note: Cat. C's equation = } 
                                1 - \\Pr \\left(y=\\text{Cat. A} \\right)
                                  - \\Pr \\left(y=\\text{Cat. B} \\right) \\)",
                          style="margin-top:15px; font-size:95%;"),
